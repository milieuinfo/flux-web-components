...
const uploadProgressSuccessUrl =
    'http://localhost:8080/iframe.html?id=components-block-upload-progress--upload-progress-success&viewMode=story';
...
describe('cypress-e2e - block components - vl-upload-progress - success story', () => {
    it('should display story', () => {
        cy.visit(uploadProgressSuccessUrl);
        cy.get('vl-upload-progress[success]').shadow();
    });
});
