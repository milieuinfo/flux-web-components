...
De twee release stages draaien sequentieel op de top-level agent en delen dus één workspace en checkout:
__verify-release__ heeft de `build/dist` output van __release-and-publish__ nodig. De scripts beslissen zelf op
basis van de branchnaam of ze effectief iets doen.
