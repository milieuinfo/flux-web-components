...
<Meta title="Planning/Aanpak v2, v3, ..."/>
...
vanuit het idee de code van Webuniversum v3 as-is af te nemen.
Dit had een aantal gevolgen:

### Element complicatie

Componenten zoals de DV [vl-ui-button](https://www.vlaanderen.be/vlaanderen-design-system/componenten/button)
moeten als een uitbreiding op een native button geïmplementeerd worden. De Flux web-component moet dus extenden van
HTMLButtonElement. De reden is dat de scss van DV enkel werkt op een `button` tag, een custom component maken met als
...
geïmplementeerd zijn, gegroepeerd in het `@domg-wc/elements` artifact.
...
- is de map nog niet conform de nieuwe conventies
- is er nog legacy code

In versie 3 (hopelijk) zal de volledige codebase consistent zijn en wordt alle legacy code geschrapt .


## v2 Release

Zolang we in v1 zaten kwamen er nieuwe 'next' componenten bij (zie
[Migratie v2 - Impact](http://localhost:8080/?path=/docs/afnemen-migratie-v2-impact--documentatie)). De bestaande
componenten bleven zoals ze geschreven waren, hun API wijzigde niet.

Bij de v2 release deden we het volgende:

- als er een oude en een nieuwe variant was van dezelfde component werd de oude geschrapt en bij de nieuwe werd
  de `-next` suffix verwijderd - de nieuwe vervangt dus de oude; hierdoor is de component niet backwards
  compatibel
- bij de componenten die behouden bleven schrappen we de `data-` prefix, zodat er in v2 nergens nog attributen zijn
  die die conventie volgen

**opmerking 1**: alle componenten die in release v2 zitten zitten ook in de laatste v1 release, er zullen
...
Net zoals we via minor en patch releases v1 hebben uitgebreid en laten evolueren zullen we dat met v2 doen. Op een
bepaald moment zullen we terug een opkuis actie willen doen die dan zal resulteren in een v3 release. Een concrete
datum of feature-set voor v3 is er nog niet.
