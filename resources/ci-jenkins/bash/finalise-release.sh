...
# op Jenkins is de checkout gedaan door een andere user (jnlp container) dan de user die dit script
# draait (root in de cypress container) - zonder safe.directory weigert git dan elke operatie
...
# verificatie: enkel uitvoeren op een hoofd-release branch (release-v<major>, bv. release-v1 of release-v2). Een
# fix-release branch zoals release-v2-2.15 mag NIET rebasen naar een develop branch - die wordt overgeslagen maar
# de stap slaagt (exit 0).
...
# op Jenkins wordt GITHUB_TOKEN gezet via withCredentials (credentialsId 'github') - nodig voor de push
if [[ -z "${GITHUB_TOKEN:-}" ]]; then
    echo "GITHUB_TOKEN is NIET gezet - kan geen geauthenticeerde push uitvoeren" >&2
    exit 1
fi

# de remote gezet door de CI server is niet geauthenticeerd, dus de remote verwijderen en eentje met authenticatie
# toevoegen; de 'x-access-token' username werkt met het GitHub App token dat Jenkins aanlevert
...
# Jenkins kan een shallow clone maken; de rebase heeft de volledige historiek van beide branches nodig
