...
const label = buttonText && buttonText.type === 'TEXT' ? escapeHtml(buttonText.textContent) : '';

// Een icoon-knop heeft geen tekstlaag en dus geen bron voor de toegankelijke naam. Het snippet toont
// een placeholder: een leeg `label` zou een knop zonder toegankelijke naam opleveren.
const iconOnlyLabel = label || '...';
...
export default {
    ...
    }${iconOnly ? ` label="${iconOnlyLabel}"` : ''}>${iconOnly ? '' : label}</vl-button>`,
    id: 'vl-button',
