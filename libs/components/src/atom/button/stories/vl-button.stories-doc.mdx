...
### CTA link als externe link

Gebruik je `cta-link` samen met het `external` attribuut, dan opent de link in een nieuw tabblad en wordt automatisch
het external icoon getoond.

Voor schermlezers voegt de component automatisch de visueel verborgen tekst "(opent in een nieuw venster)" toe. Je
hoeft dit dus niet zelf te voorzien. Gebruik je het `label` attribuut, dan vervangt dat de volledige voorgelezen tekst
en wordt de automatische melding niet toegevoegd; neem in dat geval zelf de vermelding op in het label.

<Canvas of={VlButtonStories.ButtonCtaLinkExternal}/>
...
Gebruik je een `cta-link` met het `external` attribuut, dan meldt de component zelf aan schermlezers dat de link in
een nieuw venster opent. Enkel als je het `label` attribuut zet, moet je die vermelding zelf opnemen in het label.
