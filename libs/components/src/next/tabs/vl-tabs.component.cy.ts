...
const VlTabsComponentTestUtils = {
    shouldHaveTabWithTitle: (id: string, title: string) => {
        cy.get('vl-tabs-next').shadow().find(`vl-tab-next#${id}`).find('a').find('slot').contains(title);
    },
...
const mountDefault = ({
    ...
    return cy.mount(html` <div>
        ...
        >
            <vl-tabs-pane-next id="trein" title="Trein" ?observe-title=${observeTitle}>
            ...
            <vl-tabs-pane-next id="metro" title="Metro, tram en bus" ?observe-title=${observeTitle}>
            ...
            <vl-tabs-pane-next id="fiets" title="Fiets" ?observe-title=${observeTitle}>
...
describe('component vl-tabs - attributes', () => {
    it('should render tabs view for containers with width > 767', () => {
        ...
        cy.get('vl-tabs-next').find('vl-tabs-pane-next');
        cy.get('vl-tabs-next')
            .shadow()
            .find('div#tabs > div#tabs-wrapper > ul#tab-list > vl-tab-next')
            .each(($el) => {
    ...
    it('should not have an <activeTab> selected', () => {
        ...
        cy.get('vl-tabs-next').shadow().find('a[aria-selected="true"]').should('not.exist');
    });

    it('should set the <activeTab>', () => {
        mountDefault({ ...props, activeTab: 'trein' });

        cy.get('vl-tabs-next').should('have.attr', 'active-tab', 'trein');
        cy.get('vl-tabs-next').invoke('attr', 'active-tab', 'metro');
        cy.get('vl-tabs-next').should('have.attr', 'active-tab', 'metro');
    });
    ...
    it('should set <alt>', () => {
        ...
        cy.get('vl-tabs-next').should('have.attr', 'alt');
    });

    it('should set <disableLinks>', () => {
        mountDefault({ ...props, disableLinks: true });

        cy.get('vl-tabs-next').should('have.attr', 'disable-links');
    });
});
...
describe('component vl-tabs-pane - functionality on larger devices', () => {
    ...
    it.only('should emit events on click tab', () => {
        ...
        cy.createStubForEvent('vl-tabs-next', 'vl-click');
        cy.get('vl-tabs-next').shadow().find('vl-tab-next#trein').click();
        cy.get('@change').should('have.been.called');
        cy.get('@vl-click').should('have.been.called');

        cy.get('vl-tabs-next').shadow().find('vl-tab-next#metro').click();
        cy.get('@change').should('have.been.called');
        cy.get('@vl-click').should('have.been.called');

        cy.get('vl-tabs-next').shadow().find('vl-tab-next#fiets').click();
        cy.get('@change').should('have.been.called');
        cy.get('@vl-click').should('have.been.called');
    });
    ...
    it('should show/hide content on click tab', () => {
        ...
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#metro-pane');
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#fiets-pane');

        cy.get('vl-tabs-next').shadow().find('vl-tab-next#metro').click();
        VlTabsComponentTestUtils.shouldBeVisible('vl-tab-section-next#metro-pane');
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#trein-pane');
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#fiets-pane');

        cy.get('vl-tabs-next').shadow().find('vl-tab-next#fiets').click();
        VlTabsComponentTestUtils.shouldBeVisible('vl-tab-section-next#fiets-pane');
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#trein-pane');
        VlTabsComponentTestUtils.shouldBeHidden('vl-tab-section-next#metro-pane');
    });
