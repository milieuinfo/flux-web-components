...
import * as VlTableStories from './vl-table.stories';

# Table

<FluxComponentMetaData id="components-block-table" />
...
De titels zijn gescheiden van de inhoud met een vetgedrukte lijn.<br/>
Om dit toe te passen maak je zelf gebruik van native html-attribuut `rowspan`.

<Canvas of={VlTableStories.TableJoinedRowTitles} layout="padded" />


## Expandable

Om een rij te laten uitklappen ("expanden") moet je het volgende doen:<br/>
...
<Canvas of={VlTableStories.TableExpandable} layout="padded" />
...
Als je 2 rijen hebt, en je wil rij A altijd zichtbaar zetten en rij B verborgen tot die wordt opengeklapt:<br/>
...
<Canvas of={VlTableStories.TableExpandableCustomToggleDetailsColumn} layout="padded" />
...
<Canvas of={VlTableStories.TableRowStyling} layout="padded" />

## Toegankelijkheid

Geef elke tabel een naam, via een `<caption>` in de tabel of via een `aria-label` op `vl-table`. De component is
focusbaar zodat de tabel scrollbaar blijft met het toetsenbord, en krijgt daardoor de rol `group`. Zonder naam
kondigt een schermlezer een naamloze groep aan en geraakt de gebruiker niet bij de inhoud van de tabel.

Ontbreken zowel de `<caption>` als een `aria-label`, dan valt de component terug op de naam "Naamloze tabel" en
verschijnt er een waarschuwing in de console. Die fallback is een vangnet, geen alternatief voor een eigen naam.

```html
<vl-table aria-label="Meise Botanic Garden herbarium collections">
    <table>
        ...
    </table>
</vl-table>
