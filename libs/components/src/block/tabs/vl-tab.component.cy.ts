...
const mountDefault = ({ disableLinks, id, title }: MountDefaultProps) => {
    return cy.mount(html` <ul id="tab-list" class="vl-tabs" tabs-list="" role="tablist">
        <vl-tab role="tab" id="${id}" href="${id}" ${disableLinks ? 'disable-link' : ''}>
...
describe('cypress-component - block components - vl-tab', () => {
    ...
    it('should be accessible', () => {
        ...
        cy.get('vl-tab').invoke('attr', 'role', 'none');
        cy.get('vl-tab').find('a').invoke('attr', 'aria-selected', 'false');
        cy.get('vl-tab').find('a').invoke('attr', 'role', 'tab');
...
describe('cypress-component - block components - vl-tab - attributes', () => {
    it('should handle <id> attribute correctly', () => {
        ...
        cy.get('vl-tab').should('have.id', 'new-tab');
        cy.get('vl-tab').should('have.attr', 'id', 'new-tab');
    });

    it('should handle href <attribute> correctly', () => {
        mountDefault({ ...props, id: 'new-href' });

        cy.get('vl-tab').invoke('attr', 'href', 'new-href');
    ...
    it('should handle <disable-link> attribute correctly', () => {
        ...
        cy.get('vl-tab').should('not.have.attr', 'disable-link');
    });
});
