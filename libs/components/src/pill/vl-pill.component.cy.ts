...
const mount = ({
    ...
    handleClick?: () => void
        ...
        <vl-pill
            data-cy="pill"
            data-vl-checkable=${checkable || nothing}
            data-vl-clickable=${clickable || nothing}
            data-vl-closable=${closable || nothing}
            data-vl-disabled=${disabled || nothing}
            data-vl-type=${type || nothing}
            data-vl-checked=${checked || nothing}
...
describe('component vl-pill', () => {
    ...
    PILL_TYPES.forEach((type) => it(`should contain a ${type} pill`, () => {
        mount({ type })
    ...
    it('should contain a clickable pill', () => {
        ...
        cy.getDataCy('pill').click().then(() => expect(handleClick).to.be.called)
    });
