...
<FluxComponentMetaData id="components-form-select" />


## Doel

Gebruik de `select` component om een select veld toe te voegen aan een pagina.<br/>
...
<Canvas of={VlSelectStories.SelectDeclarativeStates} />
...
Indien je declaratieve opties gebruikt bij het laden van de `vl-select`, worden deze intern ingesteld als de `initial-options` property.<br/>
...
Dit component is volledig accessible, we raden aan waar mogelijk gebruik te maken van dit component in plaats van de [vl-select-rich](/docs/components-form-select-rich--documentatie).<br/>
