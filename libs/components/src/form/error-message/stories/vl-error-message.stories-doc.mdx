...
<VluxMetaData id="components-form-error-message" />

Gebruik de `error-message` component om een error af te beelden voor een input veld.<br/>
...
```js
import { VlErrorMessageComponent } from '@domg-wc/components/form';
```

```html
<vl-error-message></vl-error-message>
```
...
De volgorde waarin de error messages getoond worden volgt dezelfde volgorde van `vl-error-message` componenten in
