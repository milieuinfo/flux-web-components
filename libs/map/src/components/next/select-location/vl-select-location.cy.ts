...
export const enterSelectLocationValue = (
    value: string,
    getSelectLocationComponent: () => Cypress.Chainable = () => cy.get('vl-select-location-next')
...
describe('vl-select-location-next', () => {
    it('heeft een div met de class .js-vl-select', () => {
        cy.mount(selectLocationFixture);
        cy.get('vl-select-location-next').shadow().find('div.js-vl-select').should('exist');
    });

    it('heeft een select html element', () => {
        cy.mount(selectLocationFixture);
        cy.get('vl-select-location-next').shadow().find('select').should('exist');
    });

    it('heeft een placeholder element met default tekst', () => {
        cy.mount(selectLocationFixture);
        cy.get('vl-select-location-next')
            .shadow()
    ...
    it('de placeholders kunnen ingesteld worden', () => {
        ...
        cy.get('vl-select-location-next')
            .shadow()
            .find('.vl-select__placeholder')
            .should('include.text', mockPlaceholder);
        cy.get('vl-select-location-next')
            .shadow()
    ...
    it('kan de geografische locatie van het geselecteerde adres opvragen', () => {
        ...
        cy.wait('@getSuggestions')
            .wait(0)
            .then(() => {
                cy.get('vl-select-location-next').shadow().find('.vl-select__item--choice').first().click();
                cy.runTestFor<VlSelectLocationComponent>(
                    'vl-select-location-next',
                    async (vlSelectLocationComponent) => {
    ...
    it('kan de geografische locatie van een Lambert-coördinaat opvragen', () => {
        ...
        cy.wait('@getLocation')
            .wait(0)
            .then(() => {
                cy.get('vl-select-location-next').shadow().find('.vl-select__item--choice').eq(1).click();
                cy.runTestFor<VlSelectLocationComponent>(
                    'vl-select-location-next',
                    async (vlSelectLocationComponent) => {
