...
Om de start van een kolom te verschuiven is er `vl-column-next--offset-2` t.e.m. `vl-column-next--offset-12` met de
responsieve varianten `vl-column-next--X-offset-N` waarbij X [m, s, xs] kan zijn.
