...
const handleRadioNodeList = (radioNodeList: RadioNodeList, value: string | string[]) => {
    Array.from(radioNodeList).forEach((radioNode) => {
        if (radioNode instanceof HTMLInputElement && radioNode.value === value && !radioNode.checked) {
            ...
            if (radioNode.validationTarget.type === 'radio') {
                ...
                if (Array.isArray(value) && value.length && radioNode.validationTarget) {
    ...
    if (Array.isArray(value) && value.length) {
        value.forEach((val) => {
            Array.from(radioNodeList).forEach((radioNode) => {
                if (radioNode instanceof VlRadioComponent && radioNode.getAttribute('value') === val) {
                    radioNode.setAttribute('checked', '');
                }
            });
        });
...
const handleVlSelect = (
    ...
    if (Array.isArray(value) && value.length) {
        const [matchingOptions, nonMatchingOptions] = partition(
            <SelectOption[]>select.options,
            (selectOption) => value.indexOf(selectOption.value) > -1
        );
        select.options = [
            ...matchingOptions.map((option) => ({ ...option, selected: true })),
            ...nonMatchingOptions.map((option) => ({ ...option, selected: false })),
        ];
    } else {
...
const handleVlCheckbox = (
    ...
    console.log('checkbox', checkbox.getAttribute('value'));
    if (Array.isArray(value) && value.length) {
        ...
        if (
            value.some((v) => {
                console.log("checkbox.getAttribute('value')", checkbox.getAttribute('value'));
